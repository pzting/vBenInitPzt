...
const pageListUserList = (() => {
  const result: any[] = [];
  for (let index = 0; index < 20; index++) {
    result.push({
      id: `${index}`,
      phone: `187239898${index}`,
      score: `12${index}`,
      loginDate: "@datetime"
    });
  }
...
export default [
   ...
   **/
  {
    url: "/basic-api/userDeal/pageListUser",
    timeout: 100,
    method: "post",
    response: ({ query }) => {
      const { page = 1, pageSize = 20 } = query;
      return resultPageSuccess(page, pageSize, pageListUserList);
    }
  },
  {
    url: "/basic-api/userDeal/addScore",
    timeout: 100,
    method: "post",
    response: () => {
      return resultSuccess(`can use`);
      // return resultSuccess({ items: standBookList });
    }
  }
] as MockMethod[];
